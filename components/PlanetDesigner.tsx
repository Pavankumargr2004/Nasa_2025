...
const PlanetDesigner: React.FC<PlanetDesignerProps> = ({ addAchievement, onBack }) => {
    ...
    const updatePlanetFromText = (text: string) => {
        ...
        if (lowerText.includes('no ring') || lowerText.includes('remove ring')) {
          setPlanetConfig(p => ({ ...p, hasRings: false }));
        } else if (lowerText.includes('ring')) {
          setPlanetConfig(p => ({ ...p, hasRings: true }));
        }
        
        if (lowerText.includes('no cloud') || lowerText.includes('remove cloud') || lowerText.includes('clear atmosphere')) {
          setPlanetConfig(p => ({ ...p, hasClouds: false }));
        } else if (lowerText.includes('cloud') || lowerText.includes('atmosphere')) {
          setPlanetConfig(p => ({ ...p, hasClouds: true }));
        }
    }
        ...
        <div className="w-full bg-black/30 rounded-2xl shadow-2xl shadow-violet-500/10 p-4 sm:p-6 backdrop-blur-sm border border-violet-700/30">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-4">
                <div className="text-center sm:text-left">
                    <h3 className="font-orbitron text-xl font-bold mb-2">Mission: Design-A-Planet</h3>
                    <p className="text-violet-300 text-sm">Chat with your AI guide, Nova, to create a unique world from your imagination!</p>
                </div>
