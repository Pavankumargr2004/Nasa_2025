    ...
    return (
        <section className="flex flex-col items-center h-full justify-center">
            ...
            <div className="w-full h-[80vh] bg-black/30 rounded-2xl shadow-2xl shadow-violet-500/10 p-2 sm:p-4 backdrop-blur-sm border border-violet-700/30 relative">
                <a-scene embedded renderer="colorManagement: true;" vr-mode-ui="enabled: false" key={selectedPlanet ? selectedPlanet.name : 'overview'} style={{ border: 'none' }}>
                    <a-assets>
                        <img id="sky" src="https://cdn.glitch.global/e843c0c0-15a2-439c-9c3f-4a699bce8042/2k_stars.jpg?v=1716335952317" alt="starry sky"/>
                    ...
                    {selectedPlanet ? (
                        ...
                        <>
                            <a-camera position="0 10 25" rotation="-20 0 0" cursor="rayOrigin: mouse;"></a-camera>
                ...
                <div className="absolute top-4 left-4 right-4 text-center pointer-events-none">
                     <h3 className="font-orbitron text-xl font-bold text-sky-300">{selectedPlanet ? selectedPlanet.name : 'Interactive Solar System'}</h3>
                     <p className="text-violet-300">{selectedPlanet ? 'Here is a closer look!' : 'Click on a planet to learn more about it!'}</p>
                </div>
                ...
                {(selectedPlanet || isLoading) && (
                    <div className="absolute bottom-4 left-4 right-4 p-4 bg-black/60 backdrop-blur-md rounded-lg border border-violet-500/50 pointer-events-auto animate-fade-in">
                        ...
                        {selectedPlanet && <h4 className="font-orbitron text-lg font-bold text-yellow-300">{selectedPlanet.name}</h4>}
